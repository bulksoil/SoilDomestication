...
pc$axes %>% 
  ggplot(aes(MDS1, MDS2, color = Compartment, shape = Type)) +
  geom_point(size = 3) +
  scale_color_manual(values = c("#377EB8", "#984EA3", "#E41A1C")) +
  theme_minimal() 

pc$axes %>% 
  ggplot(aes(MDS1, MDS2, color = Type, shape = Status)) +
  geom_point(size = 3) +
...
vDE %>% 
  ...
  mutate(Status = ifelse(Status == "-", "Autoclaved", "Live")) %>% 
