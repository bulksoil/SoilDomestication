...
h_model <- lme(Height ~ Trt * Type, random = ~1|Soil, data =  seedling_data)
h_posthoc <- glht(h_model)

anova.lme(h_aov)
tidy(TukeyHSD(h_aov))
m_aov <- aov(Mass ~ Trt * Type + Soil, data = seedling_data)
tidy(TukeyHSD(m_aov))
seedling_data %>% 
  group_by(Trt) %>% 
  nest() %>% 
  mutate(ktest = map(data, ~kruskal.test(Mass ~ factor(Type), data = .))) %>% 
  unnest(map(ktest, ~tidy(.)))

seedling_data %>% 
  group_by(Trt) %>% 
  nest() %>% 
  mutate(atest = map(data, ~lm(Mass ~ factor(Type), data = .))) %>% 
  unnest(map(atest, ~tidy(.)))
```
