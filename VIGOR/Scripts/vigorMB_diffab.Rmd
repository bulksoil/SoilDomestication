...
dds <- DESeqDataSetFromMatrix(otu2,
                                colData = map2,
                                design = ~ Shoot_length)

vsd <- varianceStabilizingTransformation(dds, blind = FALSE) %>% assay
vsd[vsd < 0.0] <- 0.0

dds <- DESeq(dds)

res <- results(dds, name = "Shoot_length") %>% tidy() %>% mutate(Trait = "Length")
```
...
zscores %>% 
  ...
  ggplot(aes(zscore, Shoot_length)) +
