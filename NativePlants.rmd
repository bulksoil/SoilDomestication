...
edgeR_plant <- nExp %>% 
  ...
  group_by(Compartment) %>% 
  nest() %>% 
  mutate(DGEL = map(data, ~tidyDGEL2(., value = "value", group = "host_common_name", method = "DESeq2")))
```
```{r}
tidyDGEL2 <- function(x, method = "edgeR", samples = "SampleID", otus = "variable", value = "value", group_column){
