...
nPhyModel_compartment %>% 
  filter(term != "(Intercept)" & term != "(phi)") %>% 
  mutate(term = gsub("Compartment", "", term)) %>% 
  mutate(p.adj = p.adjust(p.value, "BH")) %>% ungroup() %>% 
  add_count(term) %>% dplyr::rename(total_phy = n) %>% 
  filter(p.adj <= 0.05) %>% mutate(direction = ifelse(estimate > 0, "higher", "lower")) %>% 
  dplyr::count(term, total_phy, direction)
```
...
One of the prevailing phyla coming out as being specifically enriched in rice is Euryarachaeota, which contains methanogenic archaea. Let's go a big deeper here.
```{r}
