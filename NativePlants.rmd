...
intersect_edger %>% 
  ...
  inner_join(nExp, by = c("Compartment", "variable")) %>% 
  group_by(SampleID, Compartment, host_common_name) %>% 
  summarise(total = sum(RA)) %>% 
  arrange(Compartment, host_common_name) %>% 
  group_by(Compartment) %>% 
  mutate(order = 1:n()) %>% 
  ggplot(aes(x = order, y = total, fill = host_common_name)) +
  geom_bar(stat = "identity") +
  facet_wrap(~Compartment, scales = "free")

length(intersect(intersect_deseq %>% filter(Compartment == "Rhizosphere" & direction == "Rice") %>% pull(variable),
                 intersect_lm %>% filter(Compartment == "Rhizosphere" & direction == "Rice") %>% pull(variable)))
```
