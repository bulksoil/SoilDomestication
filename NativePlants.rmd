...
edgeR_plant <- nExp %>% 
  ...
  mutate(group)
  nest() %>% 
  mutate(DGEL = map(data, ~tidyDGEL2(., value = "value", group_column = "host_common_name"))) %>% 
  mutate(design = map(data, ~model.matrix(~host_common_name, tidyMB::grab_metadata(.)))) %>% 
  ...
  mutate(Mudplantain = map(fit, ~edgeR::glmLRT(., coef=c("host_common_nameMudplantain")))) %>% 
