...
```{r} 
nPC <- long_pcoa(nExp %>% group_by(variable) %>% filter(sum(value) > 0) %>% mutate(RA = RA*1000), dist = "bray", value = "RA")
nPC$axes %>% 
...
```{r}
bind_cols(plant_lm %>% mutate(model = "lm"),
          nb_glm %>% mutate(model = "glm.nb"),
          pois_glm %>% mutate(model) = "glm.nb") %>% 
  unnest(map(models, ~tidy(.))) %>% 
