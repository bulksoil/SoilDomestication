...
DESeq2_plant <- nExp %>% 
  ...
  nest() %>% 
  mutate(DGEL = map(data, ~tidyDGEL2(., value = "value", group_column = "group", method = "DESeq2", formula = "~ group"))) %>% 
...
DESeq2_plant %>% select(Mudplantain_RS, Sedge_RS, RedStem_RS, Mudplantain_ES, Sedge_ES, RedStem_ES, group_var) %>% 
  ...
  separate(taxon, sep = "_")
  mutate(direction = ifesle(estimate < 0, "Weed", "Rice")) %>% 
  filter(p.adj <= 0.05) %>% 
