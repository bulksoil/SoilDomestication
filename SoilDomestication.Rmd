...
```{r}
library(phyloseq)
dRA <- t(widen(dExp, return_df = T))
dMD <- grab_metadata(dExp, return_df = T)
...
dRA <- otu_table(dRA[match(row.names(dTax), row.names(dRA)),], taxa_are_rows = T)
dTax <- tax_table(as.matrix(dTax))
dMD <- sample_data(dMD)
physeq <- phyloseq(dRA[,1], dTax)
physeq <- merge_phyloseq(physeq, gg_tree)
physeq <- prune_taxa(taxa_names(physeq)[taxa_names(physeq)%in%unique(dom_otus$variable)], physeq)

dom.map <- data.frame(Compartment = c("aShared", rep(c("Bulk Soil", "bRhizosphere", "cEndosphere"), each = 2)),
                      Type = c("ashared",rep(c("Domesticated", "Wild"), 3)),
                      row.names = paste("D", 0:6, sep = ""))

dom.map$SampleType = paste(dom.map$Type, dom.map$Compartment)
colnames(domModSum) <- gsub("Rhizosphere" ,"bRhizosphere", colnames(domModSum))
colnames(domModSum) <- gsub("Endosphere" ,"cEndosphere", colnames(domModSum))
domModSum$aShared_ashared <- 0
domModSum$aShared_ashared[row.names(domModSum)%in%overlapping_otus$variable] <- 1
domModSum <- domModSum[,match(paste(dom.map$Compartment, dom.map$Type, sep = "_"), colnames(domModSum))]

dom.physeq <- phyloseq(dRA, 
                       dMD,
                       dTax,
                       gg_tree)

plot_tree(dom.physeq, color = "Phylum2", ladderize = "left", shape = "Compartment") +
  scale_color_manual(values = c(brewer.pal(10, "RdGy"), brewer.pal(9, "Blues"))) +
  scale_shape_manual(values = c(8, 16, 15, 17))
```


```{r}
widen2 <- function(x, samples = "SampleID", otus = "variable", value = "RA", return_df = FALSE){
